...
public class StartupService {
	...
	@PostConstruct
	@Transactional
	@TransactionConfiguration(timeout = 8000)
	public void inits() {
		long startTime = System.nanoTime();

		cacheInterface = new WildflyCache();
...
//			Map<String, Object> project = projects.get(realmCode);
			if ("FALSE".equals((String) realmUnit.getDisable().toString().toUpperCase())) {
				...
				pushBEsToCache(realm);
				pushQuestionsToCache(realm);
			}
...
//		}
	...
	private void pushBEsToCache(final String realm) {

		final List<BaseEntity> results = em
				.createQuery("SELECT a FROM BaseEntity a where a.realm=:realmStr").setParameter("realmStr", realm)
				.getResultList();
	...
	private void pushQuestionsToCache(final String realm) {
...
//						cb.equal(root.get("realm"), realm));
				...
				try {
					be = (BaseEntity)getEntityManager()
							.createQuery("SELECT a FROM BaseEntity a where a.code=:codeStr and a.realm=:realmStr").setParameter("codeStr","PRJ_" + realm.toUpperCase() ).setParameter("realmStr", realm)
							.getSingleResult();
				} catch (NoResultException nre) {
